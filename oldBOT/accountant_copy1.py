...
async def send_more(app, chat, msg):
    await app.bot.sendMessage(chat_id=chat, text=msg)
...
class Termibot:
    ...
    def cmdTRIGGER(self):
        ...
        self.updater = telegram.Bot(self.TOKEN)
    ...
    def authenticate(self,userid):
        ...
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                ...
                asyncio.run(send_more(self.application, chat=userid, msg="Click here!"))
    ...
    async def sendLINK(self,CHATID,link,txt='link'):
        await self.updater.sendChatAction(chat_id=CHATID, action="typing")
        HTML = "<a href='%s'>%s</a>"%(link,txt)
        msg = await self.updater.sendMessage(chat_id=CHATID, text=HTML ,parse_mode=ParseMode.HTML)
        self.smsID[self.dict[CHATID]].append(msg.message_id)
