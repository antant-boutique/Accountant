...
function addRow() {
  ...
  cell1.innerHTML = '<input type="text" name="products[]" id="products" required>';
...
function prefillFormFromUrl() {
  ...
  var productValues = getParameterByName('products');
  ...
  if (productValues) {
    ...
    values.forEach((value, index) => {
      if (index === 0) {
        // If it's the first value, set it in the default row
        if (productsInputs[index]) {
          productsInputs[index].value = value;
          // productsInputs[index].readOnly = true;
        }
      } else {
        // If it's not the first value, add a new row and set the value
        addRow();
        var newRowInputs = document.querySelectorAll('input[name="products[]"]');
        if (newRowInputs[index]) {
          newRowInputs[index].value = value;
          // newRowInputs[index].readOnly = true;
    newRowInputs[index].parentNode.parentNode.setAttribute('prefilled', 'true');
